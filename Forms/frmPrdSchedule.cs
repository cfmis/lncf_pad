    ...
    public partial class frmPrdSchedule : Form
    {
        ...
        private void count_datetime()
        {
            ...
            if (cmbOrder_class.SelectedIndex == 0)//白班
            {
                ...
                string am_out_time = "13:29";//早上下班
                string pm_in_time = "13:30";//下午上班
                string pm_out_time = "17:59";//下午下班
                string night_in_time = "18:00";//晚上上班
                double am_ext_time = 1, pm_ext_time = 0.5;//不是連班應扣除時間
                ...
                double defTime = string.Compare(js_time, pm_out_time);
                if ((string.Compare(ks_time, am_in_time) >= 0 && string.Compare(ks_time, am_out_time) <=0 && string.Compare(js_time, am_out_time) <=0)//08:30~12:30
                   || (string.Compare(ks_time, pm_in_time) >= 0 && string.Compare(ks_time, pm_out_time) <=0 && string.Compare(js_time, pm_out_time) <=0))//14:00~18:00
                    normal_time = sj;
                else
                {//>=08:30   >=14:00<18:00
                    if (string.Compare(ks_time, am_in_time) >= 0 && string.Compare(ks_time, am_out_time) <=0 && string.Compare(js_time, pm_in_time) >= 0 && string.Compare(js_time, pm_out_time) <=0)
                    {
                    ...
                    {//>=08:30   >=19:00
                        if (string.Compare(ks_time, am_in_time) >= 0 && string.Compare(ks_time, am_out_time) <=0 && string.Compare(js_time, night_in_time) >= 0)
                        {
                        ...
                        {//晚上加班時間//>=19:00   >=19:00
                            ...
                            {//中午開始，晚上結束>=14:00    >=19:00
                                if (string.Compare(ks_time, pm_in_time) >= 0 && string.Compare(ks_time, pm_out_time) <=0 && string.Compare(js_time, night_in_time) >= 0)
                                {
                                    str_end_time = current_day + " " + night_in_time + ":00";//結束生產時間
                                    ts = Convert.ToDateTime(str_end_time) - Convert.ToDateTime(str_start_time); //結束 - 開始 時間
                                    sj = Convert.ToSingle(ts.TotalHours.ToString());//數值型的時間
